...
public class SettingsController : MonoBehaviour
{
    ...
    {
        settingSystem.SetBGMVolume((float)value);
        UpdateBGMVolume();
    }

    void OnSoundFXVolumeChanged(float value)
    {
        settingSystem.SetSoundFXVolume((float)value);
        UpdateSoundFXVolume();
    }

    void UpdateBGMState()
    {
        bgmSlider.interactable = settingSystem.Flags[0];

        foreach (AudioSource audioSource in bgmAudioSources)
        {
            audioSource.mute = !settingSystem.Flags[0];
        }
    ...
    {
        soundFXSlider.interactable = settingSystem.Flags[1];

        foreach (AudioSource audioSource in soundFXAudioSources)
        {
            audioSource.mute = !settingSystem.Flags[1];
        }
